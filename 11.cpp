...
unsigned long long int get_cell_max(std::vector<std::vector<int>> &mat, short i, short j)
{
  unsigned long long int n = 1, s = 1, w = 1, e = 1, ne = 1, se = 1, sw = 1, nw = 1;
...
int main(int argc, char **argv)
{
  ...
  get_cell_max(mat, 0, 0);

  for (int i = 0; i < 20; i++)
    for (int j = 0; j < 20; j++)
      res = std::max(res, get_cell_max(mat, i, j));
